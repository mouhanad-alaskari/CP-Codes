                        ...
                        dbl S, dbl fa, dbl fb, dbl fc, int bottom)
{
    ...
    if (bottom <= 0 || fabs(S2 - S) <= 15*epsilon) // magic 15 comes from error analysis
        return S2 + (S2 - S)/15;
    return adaptiveSimpsonsAux(f, a, c, epsilon/2, Sleft, fa, fc, fd, bottom
                               -1) +
           adaptiveSimpsonsAux(f, c, b, epsilon/2, Sright, fc, fb, fe, bottom
                               -1);
}
...
int main()
{
    ...
    // compute integral of sin(x) from 0 to 2 and store it in the new variable I
    printf("I = %lf\n",I); // print the result
    return 0;
}
