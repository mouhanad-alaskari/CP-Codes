...
struct circle {
    ...
    circle(point cc, double rr, int ss) : c(cc), r(rr), s(ss) {
    }
...
inline bool in_circle(circle cr, point p) {
    double x = cr.c.real() - p.real(), y = cr.c.imag() - p.imag();
    double d = x * x + y * y;
    if (d < cr.r * cr.r || iseq(d, cr.r * cr.r))
        return 1;
    return 0;
}

inline bool in_circle1(circle cr, point p) {
    double x = cr.c.real() - p.real(), y = cr.c.imag() - p.imag();
    double d = hypot(x, y);
    if (d < cr.r || iseq(d, cr.r))
        return 1;
    return 0;
}
